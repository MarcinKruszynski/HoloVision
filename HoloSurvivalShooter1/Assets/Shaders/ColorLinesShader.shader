Shader "Custom/ColorLinesShader"
{
	Properties
	{
	...
	#include "UnityCG.cginc"
			...
			fixed4 frag(VertToFrag input) : SV_Target
			{
				...
				{
					ret.b = 0;
					ret.r = 1 - (abs(wpmod.y) / (_LineScale* _LinesPerMeter));
					ret.g = 0;
				}

				/*if (abs(normal.x) < 0.2f && abs(wpmod.x) < _LineScale* _LinesPerMeter)
				{
					ret.b = 0;
					ret.r = 1 - (abs(wpmod.x) / (_LineScale* _LinesPerMeter));
					ret.g = 0;
				}*/
